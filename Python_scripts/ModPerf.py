...
def ROCPlotter_Binary(real, pred, title=None, plot=True, save_name=''):
    # Инвертируем классы, если AUC < 0.5
    if metrics.roc_auc_score(real, pred) < 0.5:
        real = np.abs(real - 1)
        print('Inverted class!')

    # --- Стандартный и надежный расчет ROC-кривой с помощью sklearn ---
    fpr, tpr, _ = metrics.roc_curve(real, pred)
    auc_score = metrics.roc_auc_score(real, pred)

    # --- Построение графика с использованием объектно-ориентированного подхода ---
    fig, ax = plt.subplots(figsize=(6, 6))

    # Кривая ROC
    ax.plot(fpr, tpr, color='navy', lw=2, label='AUC = %0.3f)' % auc_score)
    ...
    # --- ИСПРАВЛЕНИЕ: Делаем сетку видимой ---
    # Устанавливаем, чтобы сетка и деления рисовались НАД фоном
    ...
    if save_name:
        # Используем объект 'fig' для сохранения, добавляем dpi и bbox_inches для лучшего качества
        fig.savefig(f'{save_name}.png', facecolor='white', transparent=False, dpi=300, bbox_inches='tight')
