...
def mixed_repeated_numeric(df, uin, group, word):
    ...
    data$uin = as.character(data$uin)
    data$group = as.character(data$group)
    data$time = as.character(data$time)

    model = lmer( val ~ group*time + (1|uin), data=data)
    emm = emmeans(model, ~ group*time)
    #print(summary(model))
    #print(emm)
    #print(pairs(emm, by = 'group', reverse = TRUE))
    #print(pairs(emm, by = 'time'))
...
def Feron_times_categoricals(df, group, word_lst, names_lst, all_pts = False):
    """
...
def shuvalov_plot(
    ...
    days_lim - ограничение таблицы по оси Y, чтобы входили все точки и обозначения
    test - тип теста ('ttest', 'umw')
