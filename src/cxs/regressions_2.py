...
class ModPerformance(object):
    ...
    def __init__(
        ...
        for i in range(len(self.real)):
            ...
            # Other params:
            brier = brier_score_loss(self.pred > t, self.real)
            se = TP_t/(TP_t + FN_t)
            ...
            g = pd.concat([g, pars], axis=1)

        g.columns = sorted(list(self.pred))
        g = g.T.reset_index()
        ...
        for i in range(iters):
            A = np.array([ROC[x] for x in np.random.randint(ROC.shape[0], size=ROC.shape[0])])
            A =np.sort(A, axis=0)[::-1]
            
            AUC_bs = 0.
            for i in range(len(self.real)):

                AUC_bs += (A[i+1,0]-A[i,0]) * (A[i+1,1]+A[i,1]) * -1
            AUC_bs *= 0.5
            bootstraps = np.append(bootstraps, [AUC_bs])

        AUC_lw = np.percentile(bootstraps, 2.5)
        AUC_up = np.percentile(bootstraps, 97.5)


        if AUC < 0.5:
            print('Real classes inverted!')
        ...
        self.auc = AUC
        self.ci = [AUC_lw, AUC_up]
        self.thresholds = g

    def results(self):  
        return(self.auc, self.ci, self.thresholds)
