...
translit <- function(strings, target_length = NULL) {
  #' Transliterate Cyrillic Strings to English and Format Them
#'
#' This function takes a vector of strings in Cyrillic, transliterates them into English, 
#' replaces non-alphanumeric characters with underscores (`_`), condenses consecutive 
#' underscores into a single one, and optionally truncates or pads the resulting strings 
#' to a specified length.
#'
#' @param strings A character vector of strings in Cyrillic to be transliterated.
#' @param target_length An optional integer specifying the desired length for each string. 
#'   If provided, the output strings will be truncated or padded with underscores to match 
#'   this length. Default is `NULL` (no truncation or padding).
#'
#' @return A character vector of transliterated and formatted strings.
#'
#' @examples
#' # Basic transliteration
#' cyrillic_strings <- c("Привет, мир!", "Как дела?", "Тестовое сообщение 123")
#' translit(cyrillic_strings)
#' 
#' # Transliteration with target length
#' translit(cyrillic_strings, target_length = 15)
#'
#' @export
  # Define a mapping for Cyrillic to English transliteration
  cyrillic_to_english <- c(
  ...
  transliterate <- function(string) {
    # Transliterate Cyrillic characters to English
    transliterated <- string
    for (cyrillic_letter in names(cyrillic_to_english)) {
      transliterated <- gsub(cyrillic_letter, cyrillic_to_english[[cyrillic_letter]], transliterated)
    }
    
    # Replace all non-alphanumeric characters to '_'
    transliterated <- gsub("[^a-zA-Z0-9]", "_", transliterated)
    
    # Remove consecutive underscores
    transliterated <- gsub("_+", "_", transliterated)
  ...
  # Apply the transliteration function to each string in the vector
  result <- sapply(strings, transliterate, USE.NAMES = FALSE)
  
  # If target_length is specified, truncate or pad each string to the target length
  if (!is.null(target_length)) {
    result <- sapply(result, function(str) {
      # Truncate if longer than target_length
      if (nchar(str) > target_length) {
        substr(str, 1, target_length)
      } else {
        # Pad with underscores if shorter than target_length
        paste0(str, paste0(rep("_", target_length - nchar(str)), collapse = ""))
      }
    }, USE.NAMES = FALSE)
  }
