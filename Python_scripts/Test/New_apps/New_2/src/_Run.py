...
SELECTED_VARS_LEN = [8, 10, 15, 20]
EDGE_THRES = 0.4
THRES_TYPE = 'weighted'
KERNEL_SIZE = [3]
INPUT_DIMS = [576, 1024, 1600, 3136]
N_EPOCHS = 1000
LEARN_RATE = [0.001, 0.0001, 0.00001, 0.000001]
MODEL_NUMBER = 0

# Load data
df = pd.read_pickle('./Quark_diast_data_06_01_2023.pkl')
...
for selected_var_len, input_dims in zip(SELECTED_VARS_LEN, INPUT_DIMS):
    ...
    class Net(nn.Module):
        ...
        def forward(self, x):
            x = F.max_pool2d(self.conv1_drop(self.conv1(x)), 3)
        ...
        for learning_rate in LEARN_RATE:
            ...
            for epoch in range(N_EPOCHS):
                ...
                if len(loss_per_epoch_val) > 0:
                    max_loss = max(loss_per_epoch_val[-30:])
            ...
            torch.save(network, save_name + '.pth')
